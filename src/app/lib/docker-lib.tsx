...
const createContainer = async ({ image, name, ports }: InstanceDocker): Promise<void> => {
...
const stopContainer = async (containerId: string): Promise<void> => {
  const docker = new Docker();
  const container = docker.getContainer(getInstanceDeploymentName({ name: containerId }));
