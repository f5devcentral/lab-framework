...
type InstancesContextType ={
  instances: Instance[] | InstanceDocker[] | InstanceK8s[] | InstanceUdf[] | null;
...
const InstancesContextProvider = ({ children }: InstancesContextProviderProps) => {

  const [instances, setInstances] = useState<Instance[] | InstanceDocker[] | InstanceK8s[] |  InstanceUdf[] | null>(null);
  ...
  const addInstance = async (instance: Instance | InstanceDocker | InstanceK8s | InstanceUdf) => {
    ...
    if (instance.type === InstanceType.Docker) {
      ...
      try {
        ...
        await createContainer({ image: instance.image, name: instance.name, ports: instance.ports } as InstanceDocker);
      } catch (error) {
    ...
    setInstances((prevInstances) => prevInstances !== null ?  [...prevInstances, instance] : [instance]);
  }
  const removeInstance = async (instance: Instance | InstanceDocker | InstanceK8s | InstanceUdf) => {
    ...
    setInstances((prevInstances) => prevInstances !== null ? prevInstances.filter(i => i.name !== instance.name) : []);
  }

  return (
    <InstancesContext.Provider value={{ instances, addInstance, removeInstance }}>
      {children}
...
const useInstancesContext = () => {
  ...
  if (!context) {
    throw new Error('useInstancesContext must be used within a InstancesContextProvider');
  }
  return context;
};

export { InstancesContextProvider, useInstancesContext, InstanceType };
export type { Instance, InstanceDocker, InstanceDockerEnv, InstanceDockerPorts, InstanceK8s, InstanceUdf };